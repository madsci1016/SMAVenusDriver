...
then
  ...
  then
	...
	wget https://github.com/madsci1016/SMAVenusDriver/archive/ESSdev2.zip
	unzip -qo ESSdev2.zip
	rm ESSdev2.zip
  ...
  template_udev_file="SMAVenusDriver-ESSdev2/install/99-candlelight.rules"
  ...
  if [ ${#serial} -eq 24 ]; then
	...
	cp -R  SMAVenusDriver-ESSdev2/dbus-sma/* ${ROOT_DIR}/data/etc/${DBUS_NAME}

  # replace inverter svg with custom yellow sunny island svg
  cp SMAVenusDriver-ESSdev2/assets/overview-inverter.svg ${ROOT_DIR}/opt/victronenergy/themes/ccgx/images
